...
esp_err_t GpioUtils::setGpioMode(const gpio_num_t gpioNum, const gpio_mode_t mode, const bool pullUp, const bool pullDown) {
    const uint64_t bitMask {(1UL << gpioNum)};
    return setGpioModeForGroup(bitMask, mode, pullUp, pullDown);
}

esp_err_t GpioUtils::setGpioModeForGroup(const uint64_t gpioBitMask, const gpio_mode_t mode, const bool pullUp, const bool pullDown) {
    gpio_config_t conf {
