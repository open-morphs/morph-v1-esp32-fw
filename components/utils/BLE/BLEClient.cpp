...
void BLEClient::gattClientEventHandler(
	...
	switch(event) {
		...
		case ESP_GATTC_SEARCH_CMPL_EVT: {
			ESP_LOGD(LOG_TAG, "GOT ESP_GATTC_SEARCH_CMPL_EVT!");
			if(m_gattc_if != gattc_if)
				break;
...
// commented out just for now to keep backward compatibility			
			...
			// }
#endif	
			ESP_LOGD(LOG_TAG, "giving m_semaphoreSearchCmplEvt...");
		...
		case ESP_GATTC_SEARCH_RES_EVT: {
			...
			ESP_LOGD(LOG_TAG, "uuid found %s", uuid.toString().c_str());
...
// TODO implement retrieving services from cache
	...
	auto waitResult {m_semaphoreSearchCmplEvt.wait(GET_SERVICES_TIMEOUT_MS, "getServices")};

	if (true == waitResult.first) {
		ESP_LOGD(LOG_TAG, "smphr received");
	}
	else {
		ESP_LOGD(LOG_TAG, "smphr timeout");
	}
		
	m_haveServices = (waitResult.second == 0);
	ESP_LOGD(LOG_TAG, "<< getServices");
	return &m_servicesMap;
} // getServices
