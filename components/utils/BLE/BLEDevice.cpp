...
/* STATIC */ bool BLEDevice::init(std::string deviceName) {
	...
	if(!initialized) {
		...
		static bool classicBtMemoryReleased {};
		if (!classicBtMemoryReleased) {
			errRc = esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT);  
			if (ESP_OK != errRc) {
				ESP_LOGE(LOG_TAG, "esp_bt_controller_mem_release failed: %d\r\n", errRc);
				return false;
			}
		}
	...
	errRc = esp_bt_controller_init(&cfg);
    if (ESP_OK != errRc) {
        ESP_LOGE(LOG_TAG, "esp_bt_controller_init: %s\n", esp_err_to_name(errRc));
        return false;
    }

	errRc = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ESP_OK != errRc) {
        ESP_LOGE(LOG_TAG, "esp_bt_controller_enable: %s\n", esp_err_to_name(errRc));
        return false;
    }
